...
/**
 * Wizard-Schritt 3: Abrechnung + Rechnungsadresse + SEPA-Mandat. Paket/Preis
 * kommen aus dem Baukasten (Schritt 2) und werden hier NICHT verändert.
 */
export function MembershipBillingForm({
  ...
  const [state, formAction] = useActionState(saveMembershipBillingAction, idleResult);
    ...
    <form action={formAction} className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-1">
          ...
          <Select id="interval_months" name="interval_months" defaultValue={String(membership?.interval_months ?? 1)}>
        ...
        <div className="space-y-1">
          ...
          <Input id="billing_day" name="billing_day" type="number" min={1} max={28} defaultValue={String(membership?.billing_day ?? 15)} />
        </div>
        <div className="space-y-1">
          <Label htmlFor="start_date">Startdatum</Label>
          <Input id="start_date" name="start_date" type="date" defaultValue={membership?.start_date ?? new Date().toISOString().slice(0, 10)} />
        </div>
        <div className="space-y-1">
          <Label htmlFor="status">Status</Label>
          <Select id="status" name="status" defaultValue={membership?.status ?? 'active'}>
        ...
        <div className="space-y-1">
          ...
          <Select id="payment_method" name="payment_method" defaultValue={membership?.payment_method ?? 'sepa'}>
      ...
      <label className="flex items-center gap-2 text-sm">
        <input type="checkbox" name="auto_send" defaultChecked={membership?.auto_send ?? false} />
      ...
      <SubmitButton>Abrechnung & SEPA speichern</SubmitButton>
    </form>
  );
}
