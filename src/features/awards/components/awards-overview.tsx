...
/**
 * Monthly awards display: employee of the month, category winners, ranking and
 * the Hall of Fame. Shared by the standalone /app/awards page (employee-facing)
 * and the Motivation-Hub „Auszeichnungen" tab (admin). `monthHrefPrefix` is the
 * URL up to (and including) `month=` so month navigation preserves the caller's
 * route/tab; `viewerId`/`viewerName` identify the current user for the running-
 * month personal view; `isAdmin` unlocks the full ranking before month end.
 */
export async function AwardsOverview({
  orgId,
  viewerId,
  viewerName,
  isAdmin,
  ...
  const canSeeFull = isAdmin || viewedIsComplete;
  ...
  const myRow: PersonScore | undefined = awards.rows.find((r) => r.userId === viewerId);

  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-bold">{de.awards.title}</h2>
          <p className="text-sm text-muted-foreground">{de.awards.subtitle}</p>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`${monthHrefPrefix}${prev}`} className="rounded-md border px-2 py-1.5 text-sm hover:bg-muted">←</Link>
          <span className="min-w-[9rem] text-center text-sm font-medium">
            {awards.monthLabel}
            {isCurrentMonth ? <span className="ml-1 text-xs text-muted-foreground">({de.awards.ongoing})</span> : null}
          </span>
          <Link href={`${monthHrefPrefix}${next}`} className="rounded-md border px-2 py-1.5 text-sm hover:bg-muted">→</Link>
        </div>
      </div>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {awards.rows.map((r, i) => (
                        <tr
                          key={r.userId}
                          className={`border-b last:border-0 ${r.userId === viewerId ? 'bg-primary/5' : ''}`}
                        >
                          <td className="py-2">
                            <div className="flex items-center gap-2">
          ...
          <Card className="border-primary/40 bg-gradient-to-br from-primary/10 to-transparent">
            ...
            <CardContent>
              <div className="mb-4 flex flex-col items-center gap-1 text-center">
                <Avatar userId={viewerId} name={viewerName} hasAvatar={false} size="lg" />
