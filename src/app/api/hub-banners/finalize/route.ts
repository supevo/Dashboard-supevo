...
/**
 * Step 3 of the hub-banner upload: records the hub_banner_images row after the
...
export async function POST(request: NextRequest) {
  ...
  const name = String(body?.name ?? '').trim().slice(0, 80) || 'Titelbild';
  const exclusive = body?.exclusive === true;
  // Exklusive Titelbilder gibt es nur über Lootbox – Level/Coin-Preis entfallen.
  ...
  // The object must live under this org's banner folder.
  const expectedPrefix = `org/${orgId}/hub-banner/`;
  if (!bannerId || !storagePath || !storagePath.startsWith(expectedPrefix)) {
    return NextResponse.json({ error: de.errors.VALIDATION }, { status: 400 });
  }

  const service = createSupabaseServiceClient();
  const { error } = await service.from('hub_banner_images').insert({
    id: bannerId,
  ...
  if (error) {
    ...
    logger.error('hub_banner.insert_failed', { error: error.message });
