...
/**
 * Step 1 of the hub-banner upload: admin-only, returns a one-time signed upload
 * target so the browser sends the image bytes DIRECTLY to storage (bypasses the
 * serverless request-body limit that made larger banners fail).
 */
export async function POST(request: NextRequest) {
  ...
  const limit = rateLimit(`banner-upload:${user.id}`, 20, 60_000);
  if (!limit.allowed) {
  ...
  const storagePath = `org/${orgId}/hub-banner/${bannerId}`;
